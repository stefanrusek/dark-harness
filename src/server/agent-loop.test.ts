// agent-loop.ts is Server's own definition of the minimal shape it needs from Core's real
...
// against agent-loop.ts's own exported types, rather than relying on it being incidentally
...
// Statically confirm FakeAgentLoop really implements every member agent-loop.ts declares —
