...
import { CONNECTION_STATUSES } from "./connection-status.constant.ts";

// DH-0149 (original coverage-backfill rationale, carried over by the DH-0157 split): this
// module needs at least one real, executed line for Bun's coverage instrumentation to
// register it — a type-only import erases before the module is evaluated, and even a
// value-level namespace import of an all-types module produces a zero-line (`LF:0`) lcov
// record that `scripts/test-isolated.ts`'s merge step silently drops. `CONNECTION_STATUSES`
// is a small genuinely-useful addition (a canonical, iterable list of `ConnectionStatus`'s
// literals) that gives this module real runtime behavior to test.
