...
### 2026-07-15 — Susan (Web domain lead), Round 3 complete — "time in current status" liveness indicator

**Chose "time in current status"** over "last event at": since the Anthropic adapter calls
`messages.create` non-streaming, no `agent_output` chunks arrive while a turn is `running` —
so "last event at" and "time since the status last changed" collapse to the same number in
practice. Framing it as "time in current status" is the more honest label for what the
number actually measures, and it generalizes cleanly if a future streaming provider adapter
starts emitting incremental output.

**The fix, end to end:**
1. `src/web/client/state.ts`: `AgentNode` gains `statusSince: string` (ISO timestamp).
   `ensureAgent` now takes the triggering event's `timestamp` and seeds `statusSince` on
   first sight of an agent; `applyEvent`'s `agent_status` case bumps `statusSince` only when
   `event.status !== node.status` (a same-status event, if one ever arrives, doesn't reset
   the clock). `seedFromTree` takes an optional `nowIso` param (defaults to
   `new Date().toISOString()`, injectable for tests) since the tree-bootstrap response
   carries no per-node timestamp of its own.
2. `src/web/client/format.ts`: new `formatElapsed(ms)` — "just now" / "42s" / "3m 12s" /
   "1h 05m", deliberately coarse (this is for operator eyeballing, not a stopwatch).
3. `src/web/client/render.ts`: `renderSidebar` and `renderAgentHeader` both take an
   optional `now: number` (defaults to `Date.now()`) and render the elapsed label — a
   `.agent-elapsed` span per row in the sidebar, a `.status-elapsed` span next to the status
   badge in the detail header. Both carry a `title` tooltip spelling out what the number
   means, since "3m 12s" alone is ambiguous without context.
4. `src/web/client/app.ts`: the actual "updates live as time passes" part. `AppDeps` gains
   `nowFn`, `setIntervalImpl`, `clearIntervalImpl` (mirroring the existing
   `setTimeoutImpl`/`clearTimeoutImpl` injection pattern used for the SSE reconnect backoff
   and error-banner timer). `start()` now also calls `startLivenessTicker()`, which fires
   `renderAll()` every 1s (`LIVENESS_TICK_MS`) via the injected interval — so a stalled
   `running` turn visibly ages in the UI even with zero new SSE events. `stop()` clears the
   interval. `renderAll()` and `bootstrapAgentTree()` both source `now`/`nowIso` from the
   same injected `nowFn` so a test can hold time fixed and advance it deterministically.
5. `src/web/client/styles.css`: `.agent-elapsed` (sidebar row) and `.status-elapsed` (detail
   header) — small, faint, monospace, consistent with the existing token-count styling.

**Regression coverage, injected clock, no real sleeps:**
- `state.test.ts`: `statusSince` seeded on first sight; bumped only on an actual status
  transition, not a same-status re-send; `seedFromTree`'s `nowIso` param seeds tree-sourced
  nodes' `statusSince` and is overridable.
- `format.test.ts`: `formatElapsed` across the "just now"/seconds/minutes/hours boundaries.
- `render.test.ts`: sidebar row and header both render the expected elapsed text for an
  injected `now`.
- `app.test.ts`: the harness's fake clock/`setIntervalImpl` (mirroring its existing fake
  `setTimeoutImpl` for the error banner) now exposes `clock.now` and `intervalCalls` — new
  test fires the injected tick directly after advancing `clock.now` by 65s with no new SSE
  event, and asserts the header's `.status-elapsed` text updates from "just now" to
  "for 1m 05s"; a second new test confirms `stop()` clears the interval.

**Not the round I expected to start on: had to re-sync the worktree first.** This worktree
(`.claude/worktrees/agent-a839f20345f4260b4`) had been created from a stale base — only the
two founding-docs commits, none of the built `src/` tree — while the coordinator branch
(`origin/claude/coordinator-onboarding-kab9ls`) already had all prior rounds merged in,
Round 3 included. Fast-forward-merged onto `origin/claude/coordinator-onboarding-kab9ls`
(`0478707`) before starting; the working tree was otherwise clean so this was a pure,
lossless catch-up, not a rebase/reset over local work.

**Gates:**
```
bun run typecheck      # tsc --noEmit && tsc --noEmit -p src/web — clean
bun run lint            # biome check . — clean, 145 files
bun run test:coverage   # 100% funcs/100% lines on every src/web/ file (695 tests project-wide)
```
`bun run e2e`: 14/18 pass; the 4 failures are this sandbox's missing tooling (no `tmux`
binary — 2 TUI/connect tests; no Chromium install at the expected path — the web e2e test;
one unrelated `security.test.ts` timeout), not a regression from this change — nothing here
touches routes, auth, or the wire protocol. Flagging for the coordinator/Hedy to confirm
against a properly provisioned e2e environment; I did not attempt to install browser/tmux
binaries into the sandbox myself.

**Deferred, not done:** everything already listed as deferred in earlier rounds still
stands (shared SSE parser extraction, SSE reconnect backoff strategy). Didn't coordinate
wording with Mary's parallel TUI round — "time in current status" is this round's framing;
