...
**Update, Round 5 — DONE.** Implemented as a `lastEventAt` (bumped on *any* event for an
agent) plus `statusSince` (bumped only on an actual status change) pair on `AgentInfo`,
derived from each `ServerSentEvent`'s ISO `timestamp` (`src/contracts/events.ts`) — no
wire-protocol change. Added `TuiState.now`, advanced only via a new `tick` action so
`render.ts` stays a pure function of state (no `Date.now()` call inside it) and tests can
inject an arbitrary clock instead of sleeping. `app.ts` drives the tick with a 1s
`setInterval` (`.unref()`d so it never keeps the process alive on its own), cleared on quit.

Presentation: tree view shows `agentId (model)  [Ns]` — elapsed since that agent's
`lastEventAt` — next to the existing status glyph; agent view's meta line shows
`Status: running (1m00s)   Last event: 12s ago`, i.e. both signals together (time-in-status
*and* last-activity), since they answer different operator questions and neither alone is
sufficient — a `running` agent mid-way through a long single tool call has an old
`statusSince` but should still show a fresh `lastEventAt` from streamed output chunks, while
a genuinely stalled agent has both going stale together. Root view was deliberately left
alone — the handoff scoped this to "tree/agent view," and root's own live output stream is
already its own liveness signal.

Added `formatElapsed` (0s / 1m05s / 2h03m style, clamped at 0 for a negative delta) as an
exported, directly-tested pure helper in `render.ts` — same "public surface for the exact
string tests need to assert against" pattern as Round 4's `CURSOR_MARKER`. New tests: a
`formatElapsed` unit suite; `state.test.ts` liveness-transition tests (event bumps
`lastEventAt` always, `statusSince` only on an actual status change, and a malformed
timestamp falls back to `Date.now()` instead of throwing) plus a `tick` reducer test; a
`render.test.ts` pair proving the tree/agent-view indicators read the right elapsed values
off an injected `now`, plus one proving an agent the client hasn't heard from yet renders
with no elapsed marker at all; an `app.test.ts` test that waits ~1.1s of real time and
asserts a redraw happened with no keystroke or SSE event in between — the one place a real
sleep was unavoidable, since the periodic tick is a real timer, not something the pure
reducer can be driven through synchronously.

All three gates green: `bun run typecheck`, `bun run lint`, `bun run test:coverage` (706
tests, 100% coverage, no regressions). `bun run e2e` has 4 pre-existing failures unrelated to
this change and outside `src/tui/` — no `tmux` binary and no headless Chromium binary
available in this environment (`e2e/tui.test.ts`'s PTY tests, `e2e/web.test.ts`), plus one
`security.test.ts` timeout — confirmed by `git status --short` showing only `src/tui/*`
