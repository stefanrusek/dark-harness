...
| [DH-0022](../DH-0022-default-server-bind-address-not-loopback.md) | `Bun.serve()` never sets `hostname`, so `dh --server` defaults to binding all interfaces, not loopback | owner triage: needs input before dispatch (ticket-triage-workflow bucket B) |
| [DH-0023](../DH-0023-web-ui-token-leak-cors-csp-clickjacking.md) | The client-served web UI's own HTTP port leaks the bearer token with no auth, plus missing CORS/CSP/clickjacking hardening | owner triage: needs input before dispatch (ticket-triage-workflow bucket B) |
...
| [DH-0022](../DH-0022-default-server-bind-address-not-loopback.md) | `Bun.serve()` never sets `hostname`, so `dh --server` defaults to binding all interfaces, not loopback 🔒 | bug | stefan |
| [DH-0023](../DH-0023-web-ui-token-leak-cors-csp-clickjacking.md) | The client-served web UI's own HTTP port leaks the bearer token with no auth, plus missing CORS/CSP/clickjacking hardening 🔒 | bug | stefan |
